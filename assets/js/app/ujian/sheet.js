...
function simpan() {
	...
	$.ajax({
		type: "POST",
		url: "http://localhost/tugas-akhir/ujian/simpan_satu",
		data: form.serialize(),
		dataType: 'json',
...
function selesai() {
	...
	$.ajax({
		type: "POST",
		url: "http://localhost/tugas-akhir/ujian/simpan_akhir/",
		data: {
			id: id_tes
		},
		beforeSend: function () {
		...
		success: function (r) {
			console.log(r);
			if (r.status) {
				window.location.href = 'http://localhost/tugas-akhir/siswa/ujian/' + id_kelas;
			}
		}
	});
}
